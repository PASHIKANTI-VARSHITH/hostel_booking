...
if (isset($_POST['room_number']) && isset($_POST['no_of_beds']) && isset($_POST['for_year'])) {
    ...
    if (empty($room_number) || empty($no_of_beds) || empty($for_year) || empty($bed_fee)) {
        ...
        $check_query = "SELECT * FROM ramanujanroom WHERE room_number = '$room_number'";
        $result = $conn->query($check_query);

        if ($result->num_rows > 0) {
            echo "<script>alert('Room number already exists');</script>";
        } else {
            $insert_query = "INSERT INTO ramanujanroom (room_number, no_of_beds, for_year, bed_fee, beds_booked, vacancy_beds) VALUES ('$room_number', '$no_of_beds', '$for_year', '$bed_fee', '0', '$no_of_beds')";
