...
if (isset($_POST['room_number']) && isset($_POST['no_of_beds']) && isset($_POST['for_year'])) {
    ...
    if (empty($room_number) || empty($no_of_beds) || empty($for_year) || empty($bed_fee)) {
        ...
        $check_query = "SELECT * FROM cvramanroom WHERE room_number = '$room_number'";
        ...
        if ($result->num_rows > 0) {
            ...
            $insert_query = "INSERT INTO cvramanroom (room_number, no_of_beds, for_year, bed_fee, beds_booked, vacancy_beds) VALUES ('$room_number', '$no_of_beds', '$for_year', '$bed_fee', '0', '$no_of_beds')";

            if ($conn->query($insert_query) === TRUE) {
                // Successfully added the room
                // You may optionaally echo a success message here
            } else {
                // Error handling
